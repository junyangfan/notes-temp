...
Vue 实现数据双向绑定主要是：采用数据劫持结合发布者-订阅者模式的方式，通过 Object.defineProperty() 来劫持各个属性的 set，get，在数据变动时发布消息给订阅者，触发相应监听回调。  
...
类组件有生命周期，可以处理一些较为复杂的逻辑，函数组件没有生命周期，需要模拟
...
Array.prototype.myReduce = function(fn, initValue) {
    const arr = this
    let total = initValue ? initValue : 0
